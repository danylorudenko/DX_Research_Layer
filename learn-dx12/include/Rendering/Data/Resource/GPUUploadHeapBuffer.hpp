...
public:
    GPUUploadHeapBuffer();
    GPUUploadHeapBuffer(ID3D12Device* device, void const* data, D3D12_RESOURCE_DESC const* resourceDesc, std::size_t size);
    ...
    void Map(void** dest, D3D12_RANGE* range);
    void Unmap(D3D12_RANGE* range);
};