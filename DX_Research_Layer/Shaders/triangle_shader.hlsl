...
float4 PS(PSInput input) : SV_TARGET
{
	...
	const float3 baseDiffColor = albedoMap.Sample(samplr, input.uv).xyz + float3(0.768f, 0.687f, 0.494f);
	...
	float3 spec = CookTorranceSpecular(fresnel, geometry, ndf, ndotv + 0.02f, ndotl) * (ndotl * 0.95f);
	...
	//float kek = ndoth;
	//return float4(kek, kek, kek, 1.0f);
